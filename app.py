...
def index():
    ...
    questions_cursor = db.execute('select questions.id as question_id, '
                                  'questions.question_text, '
                                  'askers.name as asker_name, '
                                  'experts.name as expert_name '
                                  'from questions '
                                  'join users as askers on askers.id = questions.ask_by_id '
                                  'join users as experts on experts.id = questions.expert_id '
                                  'where questions.answer_text is not null')
    questions_results = questions_cursor.fetchall()
...
def register():
    ...
    if request.method == 'POST':
        ...
        # ------getting the data from the form & adding it to the db---------#
        db = get_db()
...
def question(question_id):
    ...
    question_cursor = db.execute('select questions.question_text, '
...
def ask():
    ...
    if not user:
        return redirect(url_for('login'))
    if request.method == 'POST':
        db.execute('insert into questions (question_text, ask_by_id, expert_id) '
...
def unanswered():
    ...
    questions_cursor = db.execute('select questions.id, questions.question_text, users.name from questions '
                                  'join users on users.id = questions.ask_by_id '
...
def users():
    ...
    users_results = users_cursor.fetchall()
